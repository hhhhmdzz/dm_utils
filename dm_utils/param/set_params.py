    ...
    if isinstance(model, list):
        ...
        if mode1 == 'sklearn':
            if mode2 == 'sklearn':
                ...
                model.set_params(n_estimators=epochs, early_stopping_rounds=early_stop_rounds, verbosity=log_level)
            elif mode2 == 'lightgbm':
                log_level = get_log_level('lgb', log_level)
                model.set_params(n_estimators=epochs, verbosity=log_level, radom_state=seed)
            elif mode2 == 'catboost':
